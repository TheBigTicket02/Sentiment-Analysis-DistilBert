...
def file_to_sentences(input_path, train = True):
...
def main():
    ...
    X_test = file_to_sentences(TEST_PATH, train=False)
    train_df = create_dataframe(X, y)
    test_df = create_dataframe(X_test)
    transformer = TfidfVectorizer(stop_words='english', ngram_range=(1, 2), lowercase=True, max_features=20000)
